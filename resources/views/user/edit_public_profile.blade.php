...
@section('title', "Mana publiskā profila datu rediģēšana")
...
@section('content')
    ...
    <div class='my-16'>
        <form class='coach_info flex flex-col text-lg w-fit gap-y-4 mx-auto' action="{{ route('edit_public_profile_coach') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div>
    ...
    <script>
        ...
        personal_description.value = "{{ Auth::user()->personal_description ?? '' }}";
        contact_phone.value = "{{ Auth::user()->contact_phone ?? '' }}";
        contact_email.value = "{{ Auth::user()->contact_email ?? '' }}";
    </script>
@endsection