...
@section('content')
    ...
    <div class='flex flex-col gap-y-12 mt-12 mb-12 items-center'>
    @foreach($coaches as $coach)

        <div class='flex flex-row mx-auto w-7/12'>
            @if (isset($coach->path_to_image))
                <img class='w-3/12 border-4 border-black rounded-md' src="{{ asset('storage/' . $coach->path_to_image) }}" alt="">
            @else
                <img class='w-3/12 border-4 border-black rounded-md' src="{{ asset('storage/coaches_profile_pictures/default_profile_picture.jpg') }}" alt="">
            @endif
