...
@section('content')
    ...
    <form action="{{ route('edit_group_training') }}" method="POST" class='flex flex-col gap-y-4 w-6/12 mt-12 mb-8 mx-auto' enctype="multipart/form-data">
        ...
        <div class='flex flex-col'>
            ...
            <input type="text" name='title' class='rounded-md' maxlength="50" required value="{{ old('title') ?? $group_training->name }}">
            @if ($errors->has('title'))
        ...
        <div class="flex flex-col">
            ...
            <textarea name="description" cols="30" rows="5" required class='rounded-md'>{{ old('description') ?? $group_training->description }}</textarea>
        ...
        <div>
            ...
            @for ($i = 0; $i < count($days_lv); $i++)
                ...
                @php
                    $training_this_day = false;
                    if (array_key_exists($days_eng[$i], $group_training->schedule)) {
                        $training_this_day = true;
                    }
                @endphp

                <div class='flex flex-row items-center justify-between gap-x-2 text-lg'>
                    <div class='flex flex-row items-center gap-x-4'>
                        <input type="checkbox" name="{{ $days_eng[$i] }}" id="" data-day="{{ $days_eng[$i] }}" class="day_checkbox" @if($training_this_day) checked @endif>
                    ...
                    <div class='flex flex-row gap-x-4'>
                        <div class='flex flex-col'>
                            ...
                            <input type="time" name="start_time_{{ $days_eng[$i] }}" id="" @if (!$training_this_day) disabled @endif class='start_time_{{ $days_eng[$i] }} rounded-md @if (!$training_this_day) cursor-not-allowed @endif' value="{{ $group_training->schedule[$days_eng[$i]]['start'] ?? null }}">
                        </div>

                        <div class='flex flex-col'>
                            <label for="end_time_{{ $days_eng[$i] }}">Nodarbības beigas:</label>
                            <input type="time" name="end_time_{{ $days_eng[$i] }}" id="" @if (!$training_this_day) disabled @endif class='end_time_{{ $days_eng[$i] }} rounded-md @if (!$training_this_day) cursor-not-allowed @endif' value="{{ $group_training->schedule[$days_eng[$i]]['end'] ?? null }}">
                        </div>
                    </div>
                </div>
            @endfor
        ...
        <div class="flex flex-col">
            ...
            <input class='w-fit' type="number" name="max_participants" min="10" max="50" required value="{{ old('max_participants') ?? $group_training->max_clients }}">
        </div>
        ...
        @else
            ...
            <select name="coach_id" required class='w-fit'>
                @foreach ($coaches as $coach)
                    <option @if ($group_training->coach_id === $coach->coach_id) selected @endif value="{{ $coach->coach_id }}">{{ $coach->name }}  {{ $coach->surname }} (p.k. {{ $coach->personal_id }})</option>
                @endforeach
            </select>
        @endif

        <input type="hidden" name='training_id' value="{{ $group_training->training_id }}">
        <x-main_button type='submit' class='mt-8 text-lg w-1/2 mx-auto p-4'>Saglabāt izmaiņas</x-main_button>
    </form>
