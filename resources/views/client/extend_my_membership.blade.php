...
@section('title', 'Pagarināt manu abonementu')
...
<h1 class='text-center font-bold text-3xl mt-12'>Pagarināt manu abonementu</h1>
...
<form id='extend_membership_form' action="{{ route('extend_client_membership') }}" method="POST" class='hidden'>
    @csrf
    <input type="hidden" name='payment_completed' value="1">
    <input type="hidden" name="client_id" value="{{ Auth::user()->client_id }}">
    <input type="hidden" name="membership_name" id='extend_form_membership_name'>
...
<script>
    ...
    amount.textContent = selected_membership_price;
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (error) {
            ...
            const extend_membership_form = document.querySelector('#extend_membership_form');
            document.querySelector('#extend_form_membership_name').value = selected_membership;
            document.querySelector('#status').value = paymentIntent.status;
            extend_membership_form.submit();
        }
    });
